...
class StackSummary(list):
    ...
    def extract(klass, frame_gen, *, limit=None, lookup_lines=True,
        ...
        for f, lineno in frame_gen:
            ...
            fnames.add(filename)
            print("traceback2:  f.f_globals", filename, f.f_globals.items(), '__loader__' in f.f_globals)
    ...
    def format(self, *, chain=True):
        ...
        if chain:
            if self.__cause__ is not None:
                for v in self.__cause__.format(chain=chain):#yield from self.__cause__.format(chain=chain)
                    yield v
                yield _cause_message
            elif (self.__context__ is not None and
                not self.__suppress_context__):
                for v in self.__context__.format(chain=chain):#yield from self.__context__.format(chain=chain)
                    yield v
                yield _context_message
        if self.exc_traceback is not None:
            yield 'Traceback (most recent call last):\n'
        for v in self.stack.format(): #yield from self.stack.format()
            yield v
        for v in self.format_exception_only(): #yield from self.format_exception_only()
            yield v
